...
UNITY_DOTS_INSTANCING_START(MaterialPropertyMetadata)
    ...
    UNITY_DOTS_INSTANCED_PROP(float , _GrassTiling)
    UNITY_DOTS_INSTANCED_PROP(float , _GroundTiling)
...
inline void InitializeStandardLitSurfaceData(float2 uv, float2 uvWS, out SurfaceData outSurfaceData)
{
    half blendMask = BlendMap(uv, uvWS);
    half4 groundCol = SampleAlbedoAlpha(uvWS * _GroundTiling, TEXTURE2D_ARGS(_GroundTex, sampler_GroundTex));
    half4 grassCol = SampleAlbedoAlpha(uvWS * _GrassTiling, TEXTURE2D_ARGS(_GrassTex, sampler_GrassTex));
    half4 col = lerp(groundCol, grassCol, blendMask);
    outSurfaceData.alpha = 1;//Alpha(col.a, _BaseColor, _Cutoff);

    half4 specGloss = SampleMetallicSpecGloss(uv, col.a);
    outSurfaceData.albedo = col.rgb;
...
#ifdef _NORMALMAP
    half3 groundNormal = SampleNormal(uvWS * _GroundTiling, TEXTURE2D_ARGS(_GroundNormalTex, sampler_GroundNormalTex), 1);
    half3 grassNormal = SampleNormal(uvWS * _GrassTiling, TEXTURE2D_ARGS(_GrassNormalTex, sampler_GrassNormalTex), 1);
    half3 normal = lerp(groundNormal, grassNormal, blendMask);
#else
    normal = half3(0, 0, 1);
#endif
