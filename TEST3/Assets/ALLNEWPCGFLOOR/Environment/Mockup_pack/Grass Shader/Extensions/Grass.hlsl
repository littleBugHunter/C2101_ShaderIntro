...
#include "Extensions/Distribution.hlsl"
...
Varyings vert(Attributes input) {
	...
	output.positionWS = vertexInput.positionWS;// + float4(0, 0, 5, 0);
...
{
	half blendMask = pow(BlendMap(input[0].uv, input[0].uvWS), _Falloff);
	...
	float height = _Height * (1 + _RandomHeight * (rand(positionWS.yxz) - 0.5)) * blendMask;
	...
	if(blendMask > 0)
	{
		...
		output.positionWS = positionWS + mul(transformMatrixWithWind, float3(0, bend, height)) * blendMask;
		output.positionCS = WorldToHClip(output.positionWS, normalWS);
